...
module "router" {
  ...
  port_forwards = {
    ...
    "http_secondary" = { # Define a port forward rule for the second VM
      external_port = 51001
      internal_port = 80
      internal_ip   = module.Secondary.ip
    }
  }
}
module "Main" {
...
output "secondary" {
  value = "ssh -o StrictHostKeyChecking=no -o UserKnownHostsFile=/dev/null -J root@${module.router.router-ip} root@${module.Secondary.ip}"
}
